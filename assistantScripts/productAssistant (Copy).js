...
(function () {
  ...
  const fillDescriptionFromPasteEditor = () => {
    ...
    // Clean backgrounds and inline text colors — let the page CSS handle colors
    let html = cleanBackgroundFromHtml(pasteEditor.innerHTML);

    // We must bypass Quill entirely to preserve raw HTML (h1, h2, etc.).
    // Quill's dangerouslyPasteHTML strips/converts tags to its own format.
    // Strategy: disconnect Quill's MutationObserver, set innerHTML directly,
    // then reconnect it — so Quill never sees the mutation.
    const quill = container.__quill;
    if (quill && quill.scroll && quill.scroll.observer) {
      // Disconnect observer, set HTML, reconnect
      quill.scroll.observer.disconnect();
      ed.innerHTML = html;
      // Reconnect observer on the editor element
      quill.scroll.observer.observe(ed, {
        attributes: true,
        characterData: true,
      ...
      // Force Quill to rebuild its delta from the current DOM
      try {
        quill.update("silent");
      } catch (e) {
        // Quill may throw if delta doesn't match perfectly — safe to ignore
      }
    } else {
      // Fallback if Quill instance not accessible:
      // Use a synthetic paste event so Quill processes it through clipboard
      ed.focus();
    ...
    // Sync to hidden field after the DOM settles
    setTimeout(() => {
      const h = document.getElementById("specification");
      if (h) h.value = correctULTagFromQuill(ed.innerHTML);
    }, 300);
